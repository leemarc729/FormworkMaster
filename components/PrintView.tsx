...
import { Printer, ArrowLeft, Download, FileCheck } from 'lucide-react';
...
const PrintView: React.FC<Props> = ({ data, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 pb-10">
      ...
      <div 
        ref={componentRef}
        className="max-w-[21cm] mx-auto bg-white shadow-2xl p-[2cm] min-h-[29.7cm] print:shadow-none print:m-0 print:w-full print:max-w-none"
      >
        ...
        <section className="mb-12 page-break-inside-avoid">
           ...
           <ol className="list-decimal pl-6 space-y-3 text-justify leading-relaxed">
             {data.clauses.filter(c => c.selected).map((clause) => (
               <li key={clause.id}>
                 <span className="font-bold text-gray-900">
                    {clause.category === 'custom' ? '【特別約定】' : ''}
                    {clause.category === 'payment' ? '【付款辦法】' : ''}
                    {clause.category === 'safety' ? '【安全衛生】' : ''}
                 </span>
                 {clause.content}
               </li>
             ))}
